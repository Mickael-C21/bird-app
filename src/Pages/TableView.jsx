...
const SORTS = [
  ...
  { v: "taille",    l: "Taille"    },
  { v: "poids",     l: "Poids"     },
  { v: "envergure", l: "Envergure" },
  { v: "longevite", l: "Longévité" },
];

// Photos Wikimedia par nom normalisé
const BIRD_PHOTOS = {
...
const fmt = (g) => !g ? "—" : g >= 1000 ? `${(g/1000).toFixed(1)} kg` : `${g} g`;
...
export default function Tableau() {
  ...
  const filtered = birds
    .filter((b) => {
      ...
      return (
        (b.nom_commun?.toLowerCase().includes(q) || b.nom_scientifique?.toLowerCase().includes(q)) &&
    ...
    .sort((a, b) => {
      ...
      if (sort === "envergure") return (a.envergure_cm  ?? 0) - (b.envergure_cm  ?? 0);
      if (sort === "longevite") return (a.longevite_ans ?? 0) - (b.longevite_ans ?? 0);
    ...
    <div className="page-wrap">
      <div className="banner">
        <div className="banner-inner">
          ...
          <p>Vue synthétique des caractéristiques physiques et de conservation — cliquez pour voir le détail</p>
        </div>
      </div>
      ...
      <div className="page-inner">
        <div className="toolbar">
          ...
          {SORTS.map((s) => (
            <button
              key={s.v}
              className={`chip ${sort === s.v ? "active" : ""}`}
              onClick={() => setSort(s.v)}
            >
              {s.l}
            </button>
        ...
        <p className="count">{filtered.length} espèce(s)</p>
            ...
            {filtered.map((bird) => {
              ...
              return (
                <div
                  ...
                  title={`Voir les détails de ${bird.nom_commun}`}
                >
                  ...
                  <div className="t-card-body">
                    <div className="t-card-head">
                      ...
                      {sc && <span className={`badge badge-${sc}`}>{sc}</span>}
                    </div>
                    <p className="t-card-sci">{bird.nom_scientifique}</p>
                    <div className="t-stats">
                      <div>
                        <div className="t-stat-label">Taille</div>
                        <div className="t-stat-value">{bird.taille_cm ? `${bird.taille_cm} cm` : "—"}</div>
                      </div>
                      <div>
                        <div className="t-stat-label">Poids</div>
                        <div className="t-stat-value">{fmt(bird.poids_g)}</div>
                      </div>
                      <div>
                        <div className="t-stat-label">Envergure</div>
                        <div className="t-stat-value">{bird.envergure_cm ? `${bird.envergure_cm} cm` : "—"}</div>
                      </div>
                      <div>
                        <div className="t-stat-label">Longévité</div>
                        <div className="t-stat-value">{bird.longevite_ans ? `${bird.longevite_ans} ans` : "—"}</div>
                      </div>
                    </div>
                  </div>
                </div>
